...
最后执行action，可以看到，第二个例子中，当返回f2（）这个对象的时候，f1（）早已经没用了，原话是不在激活状态了，但是f2（）依然可以找到f1（）的映射，也是有效的，这个例子很关键，在此标记一下，这里的关键不在于为什么只是返回了f2没有执行却依然可以打印出88，而是因为在返回了f2后，f1已经无用的情况下，f2依然可以自动映射到f1中的X变量。（说的有点绕，我其实也似懂非懂，不过大概意思是知道的，以后回头再看一下）
